...
def build_ansatz(ansatz: str, ansatz_params: dict, num_qubits: int, backend: BackendV2) -> tuple[QuantumCircuit, dict | None]:
    ...
    print(f"[DEBUG] Building ansatz '{ansatz}' with num_qubits = {num_qubits}")
    ...
    def apply_entanglement_map(entanglement):
        ...
        if entanglement == 'color':
            assert backend.coupling_map.is_symmetric, 'Non-symmetric coupling map. Do you mean `di-color`?'
            coupling_map = [(i,j) for i,j in backend.coupling_map if i < j]
        elif entanglement == 'di-color':
            coupling_map = list(backend.coupling_map)
            assert not backend.coupling_map.is_symmetric, 'Symmetric coupling map. Do you mean `color`?'
    ...
    if ansatz == 'TwoLocal':
        ...
        # Debug check: Ensure ansatz has qubits before measuring
        print(f"[DEBUG] Ansatz created with {ansatz_.num_qubits} qubits before measure_all()")
        # if ansatz_.num_qubits == 0:
        #     raise ValueError("Ansatz has zero qubits; cannot measure.")
        ansatz_.measure_all()
...
def problem_mapping(lp_file: str, ansatz: str, ansatz_params: dict, theta_initial: str, device: str, instance: str):
    ...
    print(f"[DEBUG] Calling build_ansatz with num_qubits={num_vars}")
